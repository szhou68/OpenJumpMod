...
//            finalGeometry = (Geometry) geometry.get(0);
  ...
  public void startElement(String uri, String name, String qName,
    ...
    try {
      ...
      if ((STATE == STATE_WAIT_FEATURE_TAG)
    	...
    	// szhou20170627 - check for columns defined as attributes in featureTag
    	  if(atts!=null){
    		  for(int i = 0; i < atts.getLength();++i){
    			  String attNm = atts.getLocalName(i);
    			  int index = GMLinput.match(qName, attNm, atts); 
    			  if(index > -1){
    				  currentFeature.setAttribute(GMLinput.columnName(index),
    						  GMLinput.getColumnValue(index, tagBody.toString(), atts));
    			  }
    		  }
    	  }// end mod szhou
  ...
  private int parseSRID(String srsName) {
    try {
      // szhou20170627 - for OSMM only
      if(srsName.equalsIgnoreCase("osgb:BNG")){
    	  return 27700;
      }
